...
public class Verification
{
	...
	public void existuser(String uid,String pwd)
	{
		try
		{
			...
			String sql="select username,machineip,port from Firewall where username='"+uid+"' and password='"+pwd+"'";
			...
			else
			{
			 	...
			 	String sq="update Firewall set status='yes',machineip='"+na+"' where username='"+str+"'";
				System.out.println(sq);
				int s2=st2.executeUpdate(sq);
				Thread.sleep(500);
				Listen l=new Listen(port,str);
				l.start();			
			   	Firewall s=new Firewall(str,port);
				s.show ();
		   		s.setSize(400,547);
		   		s.setTitle("Firewall");
